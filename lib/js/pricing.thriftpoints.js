  ...
  function calculatePrice(){
    ...
    for( const [key,value] of Object.entries(package.modules) ){
      if( value ){
        switch(key){
          case 'emailMarketing':
            totalPrice = totalPrice + pricing.modules[key].monthly + ( pricing.modules.emailMarketing.additonalAudience * (package.numAudiences - 1) );
            break;
    ...
    if( completeManagement && ( package.numAudiences > 1 ) )
      totalPrice = totalPrice + ( pricing.modules.emailMarketing.additonalAudience * ( package.numAudiences - 1 ) );
    ...
    if( 0 < totalPrice ){
      ...
      // Starter Pack (Google Ads, BrightSign, Social Media)
      if( package.modules.googleAds && package.modules.brightSign && package.modules.socialMedia && ! package.modules.emailMarketing )
        formattedTotalPriceDisplay = '<div class="info"><h3>Starter Pack</h3><p>Google Ads + BrightSign (for 2 stores) + Social Media</p></div>' + formattedTotalPriceDisplay;
      if( package.modules.googleAds && package.modules.brightSign && package.modules.socialMedia && package.modules.emailMarketing )
        formattedTotalPriceDisplay = '<div class="info"><h3>Complete Management</h3><p>Google Ads + BrightSign (for 2 stores) + Social Media + Email Marketing (1 audience, additonal audiences $500/ea/month)</p></div>' + formattedTotalPriceDisplay;
