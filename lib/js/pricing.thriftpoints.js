  ...
  function calculatePrice(){
    ...
    for( const [key,value] of Object.entries(package.modules) ){
      if( value ){
        switch(key){
          case 'brightSign':
            monthly = pricing.modules[key].monthly
            if( 1 < package.numLocations )
              monthly = monthly + ( 50 * (package.numLocations - 1) );
            totalPrice = totalPrice + monthly;
            break;
          ...
          default:
            monthly = (pricing.modules[key].perLocation)? pricing.modules[key].monthly * package.numLocations : pricing.modules[key].monthly ;
            totalPrice = totalPrice + monthly;
        }
