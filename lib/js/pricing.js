...
jQuery(document).ready(function($){
  ...
  function calculatePrice(){
    ...
    // ✅ Override GF Total with our computed number and force a price recalculation
    window.smcCustomTotal = Math.round(initialBankDraft * 100) / 100; // raw number (no $/commas)
    registerTotalOverride();
    if (typeof window.gformCalculateTotalPrice === 'function') {
      window.gformCalculateTotalPrice(Number(wpvars.formId));
    } else {
    ...
    $('.smco-pricing-form #input_' + wpvars.formId + '_22').val(formattedInitialBankDraft);   // Today's Bank Draft
    $('.smco-pricing-form #input_' + wpvars.formId + '_23').val(formattedRemainingBankDraft); // Remaining Bank Draft
  }
