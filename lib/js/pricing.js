...
jQuery(document).ready(function($){
  ...
  // --- GF Total override setup -------------------------------------------
  window.smcCustomTotal = null;                        // raw number we want GF Total to show
  window.smcTotalFilterRegistered = window.smcTotalFilterRegistered || false;
  
  function registerTotalOverride(){
    if (window.smcTotalFilterRegistered) return;       // avoid double-register
    if (typeof gform !== 'undefined' && typeof gform.addFilter === 'function') {
      gform.addFilter('gform_product_total', function (total, id) {
        if (Number(id) !== Number(wpvars.formId)) return total;
        var override = (typeof window.smcCustomTotal === 'number' && !isNaN(window.smcCustomTotal)) ? window.smcCustomTotal : null;
        console.log('[pricing.js] gform_product_total — incoming:', total, 'override:', override);
        return (override !== null) ? override : total;
      });
      window.smcTotalFilterRegistered = true;
    }
  }

  // Register the filter after the form renders, then force an initial calc
  jQuery(document).on('gform_post_render', function (event, formId) {
    if (Number(formId) !== Number(wpvars.formId)) return;
    registerTotalOverride();
    if (typeof window.gformCalculateTotalPrice === 'function') {
      window.gformCalculateTotalPrice(Number(wpvars.formId));
    } else {
      jQuery(document).trigger('gform_price_change', [Number(wpvars.formId)]);
    }
  });
  // -----------------------------------------------------------------------
  ...
  function calculatePrice(){
    ...
    // ✅ Override GF Total with monthly recurring fee only
    window.smcCustomTotal = Math.round(totalMonthlyPrice * 100) / 100; // raw number (no $/commas)
    registerTotalOverride();
    if (typeof window.gformCalculateTotalPrice === 'function') {
      window.gformCalculateTotalPrice(Number(wpvars.formId));
    } else {
    ...
    $('.smco-pricing-form #input_' + wpvars.formId + '_19').val(formattedOnetimePrice);       // One-Time/Setup Fee
    $('.smco-pricing-form #input_' + wpvars.formId + '_21').val(formattedMonthlyPrice);       // Recurring Fee
    $('.smco-pricing-form #input_' + wpvars.formId + '_22').val(formattedInitialBankDraft);   // Today's Bank Draft (for record only)
    $('.smco-pricing-form #input_' + wpvars.formId + '_23').val(formattedRemainingBankDraft); // Remaining Bank Draft (for record only)
  }
